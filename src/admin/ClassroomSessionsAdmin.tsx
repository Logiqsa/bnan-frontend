...
import {
  referenceId,
  referenceName,
} from "@/admin/zoom/classroomZoomNormalization";
...
const statusLabelsEn: Record<string, string> = {
  ...
  cancelled: "Cancelled",
  scheduled: "Scheduled",
  awaiting_zoom_end: "Waiting for Zoom to end",
};
...
const kindLabelsEn: Record<string, string> = {
  manual_recording: "Uploaded recording",
  live: "Live lesson",
  scheduled: "Scheduled lesson",
};
...
const nestedName = (
  value: ClassroomSession["subject"] | ClassroomSession["teacher"],
) => {
  if (typeof value === "string") return isObjectId(value) ? "" : value;
  return (
    value?.name || (value && "fullName" in value ? value.fullName : "") || ""
  );
};
...
export default function ClassroomSessionsAdmin() {
  ...
  const filteredClassrooms = useMemo(
    () =>
      classrooms.filter(
        (item) =>
          (curriculumId === "all" ||
            referenceId(item.curriculum) === curriculumId) &&
  ...
  const statuses = useMemo(
    () =>
      Array.from(
        new Set(sessions.map((item) => item.status).filter(Boolean)),
      ) as string[],
    [sessions],
  );
      ...
      .catch((error) =>
        toast.error(
          (error as Error).message ||
            pick("تعذر تحميل الفصول", "Unable to load classes"),
        ),
      )
  ...
  useEffect(() => {
    catalogApi
      ...
      .catch((error) =>
        toast.error(
          (error as Error).message ||
            pick("تعذر تحميل المناهج", "Unable to load curricula"),
        ),
      );
  }, [pick]);
  ...
  useEffect(() => {
    ...
    catalogApi
      ...
      .catch((error) => {
        ...
        toast.error(
          (error as Error).message ||
            pick("تعذر تحميل الصفوف", "Unable to load grades"),
        );
      });
  }, [curriculumId, pick]);
  ...
  const loadSessions = useCallback(
    async (id: string) => {
      ...
      try {
        const [sessionsResponse, subjectsResponse, recordingsResponse] =
          await Promise.all([
        ...
        toast.error(
          (error as Error).message ||
            pick("تعذر تحميل التسجيلات", "Unable to load recordings"),
        );
      } finally {
  ...
  useEffect(() => {
    if (!highlightedSessionId || loadingSessions || sessions.length === 0)
      return;
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-6xl" dir={isArabic ? "rtl" : "ltr"}>
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-3xl font-bold">
              {pick("تسجيلات الفصل", "Class recordings")}
            </h1>
            <p className="mt-1 text-muted-foreground">
              {pick(
                "اعرض وفلتر كل الحصص الخاصة بالفصل.",
                "View and filter all lessons for a class.",
              )}
            </p>
          </div>
          <Button
            variant="outline"
            className="gap-2"
            disabled={!classroomId || loadingSessions}
            onClick={() => void loadSessions(classroomId)}
          >
            <RefreshCw
        ...
        <Card className="mb-6">
          <CardContent className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-5">
            <div className="space-y-2">
              ...
              <Select
                value={curriculumId}
                onValueChange={chooseCurriculum}
            ...
            <GradeStageFilter
              grades={grades}
              gradeId={gradeId}
              mode={selectedCurriculum?.registrationMode}
              allowAll
              disabled={loadingClassrooms || curriculumId === "all"}
              onGradeChange={(value) => {
                setGradeId(value);
                setClassroomId("");
              }}
              ...
              <Popover open={classroomOpen} onOpenChange={setClassroomOpen}>
                    ...
                    <span className="truncate">
                      {loadingClassrooms
                        ...
                        : selectedClassroom?.name ||
                          pick("اختر أو ابحث", "Select or search")}
                    </span>
                ...
                >
                  <Command dir={isArabic ? "rtl" : "ltr"}>
                    ...
                    <CommandList>
                      <CommandEmpty>
                        {pick("لا توجد نتائج.", "No results found.")}
                      </CommandEmpty>
                      <CommandGroup>
                          ...
                          >
                            ...
                            <span>
                              ...
                              <span className="text-xs text-muted-foreground">
                                {[
                                  referenceName(item.curriculum),
                                  referenceName(item.grade),
                                ]
                                  .filter(Boolean)
                                  .join(" — ")}
                              </span>
            ...
            <div className="space-y-2">
              <Label>{pick("حالة السيشن", "Session status")}</Label>
              <Select
                value={status}
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="all">
                    {pick("كل الحالات", "All statuses")}
                  </SelectItem>
                  {statuses.map((item) => (
                    <SelectItem key={item} value={item}>
                      {(isArabic ? statusLabelsAr : statusLabelsEn)[item] ||
                        item}
                    </SelectItem>
                  ))}
                </SelectContent>
        ...
        {!classroomId ? (
          <Card>
            <CardContent className="grid min-h-56 place-items-center text-center text-muted-foreground">
              <div>
                <CalendarDays className="mx-auto mb-3 h-9 w-9 opacity-40" />
                <p>
                  {pick(
                    "اختر فصلًا لعرض التسجيلات.",
                    "Select a class to view its recordings.",
                  )}
                </p>
              </div>
            </CardContent>
          ...
          <Card>
            <CardContent className="grid min-h-56 place-items-center text-muted-foreground">
              {pick("لا توجد تسجيلات مطابقة.", "No matching recordings.")}
            </CardContent>
          </Card>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {visibleSessions.map((session, index) => {
              const sessionSubjectId =
                ...
                (typeof session.classroomSubject === "string"
                  ? session.classroomSubject
                  : session.classroomSubject?.id);
              const subjectDetails = subjects.find(
                (item) =>
                  item.classroomSubjectId === sessionSubjectId ||
                  item.id === sessionSubjectId,
              );
              const populatedClassroomSubject =
                typeof session.classroomSubject === "object"
                  ? session.classroomSubject
                  : null;
              const subject =
                nestedName(session.subject) ||
                populatedClassroomSubject?.subject?.name ||
                populatedClassroomSubject?.name ||
                subjectDetails?.name ||
                subjectDetails?.subject?.name;
              const teacher =
                nestedName(session.teacher) ||
              ...
              const matchingRecording = recordings.find(
                (item) =>
                  ...
                  normalizeSessionName(item.sessionName) ===
                    normalizeSessionName(session.title || session.sessionName),
              );
              const recording = matchingRecording?.localUrl;
              const recordingProcessing = Boolean(
                matchingRecording && !recording,
              );
              const sessionName =
                session.title ||
                session.sessionName ||
                pick("حصة بدون اسم", "Untitled lesson");
              const openRecording = () => {
                if (!recording) return;
                setSelectedRecording({ sessionName, recordingLink: recording });
              };
              return (
                <Card
                  ...
                  aria-label={
                    recording ? `مشاهدة تسجيل ${sessionName}` : undefined
                  }
                  ...
                  className={cn(
                    ...
                    recording &&
                      "cursor-pointer hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                    highlightedSessionId &&
                      sessionId === highlightedSessionId &&
                      "border-primary bg-primary/10 ring-4 ring-primary/25 shadow-lg",
                  )}
                >
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h2 className="font-bold">{sessionName}</h2>
                        {subject && (
                      ...
                      >
                        {(isArabic ? statusLabelsAr : statusLabelsEn)[
                          session.status || ""
                        ] ||
                          session.status ||
                          pick("غير محددة", "Unknown")}
                      </Badge>
                    </div>
                    <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm">
                      <span>
                        {session.startAt
                          ...
                          : pick("بدون موعد", "No date")}
                      </span>
                      {session.sessionKind && (
                        <span className="text-muted-foreground">
                          {(isArabic ? kindLabelsAr : kindLabelsEn)[
                            session.sessionKind
                          ] || session.sessionKind}
                        </span>
                      )}
