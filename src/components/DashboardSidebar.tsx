...
export const roleNavItems: Record<string, NavItem[]> = {
  admin: [
    ...
    {
      label: "حسابات زوم",
      labelEn: "Zoom accounts",
      icon: Video,
      path: "/admin?tab=zoom-accounts",
    },
    {
      label: "ربط الصفوف بـ Zoom",
    ...
    {
      label: "رفع تسجيل حصة",
      labelEn: "Upload lesson recording",
      icon: Upload,
    ...
    },
    {
      ...
      icon: Award,
      path: "/admin?tab=success-stories",
    },
    {
...
const SidebarContent = ({
  ...
  return (
    <div
      className="h-full min-h-0 bg-sidebar text-sidebar-foreground flex flex-col"
      dir={isArabic ? "rtl" : "ltr"}
    >
      <div
        className={`border-b border-sidebar-border transition-all duration-300 ${collapsed ? "p-2" : "p-4"}`}
      >
        <div className="flex items-center gap-1">
          <Link
            to="/"
            aria-label="العودة إلى الصفحة الرئيسية"
            className={`flex justify-center overflow-hidden transition-[width] duration-300 ${collapsed ? "min-w-0 flex-1" : "w-4/5"}`}
          >
            <img
              src={logo}
              alt="أكاديمية بنان"
              className={`max-w-full object-contain brightness-0 invert transition-all duration-300 ${collapsed ? "h-10" : "h-20"}`}
            />
          </Link>
          {onToggle && (
            <button
              ...
              aria-label={
                collapsed
                  ? pick("فتح القائمة الجانبية", "Expand sidebar")
                  : pick("طي القائمة الجانبية", "Collapse sidebar")
              }
              title={
                collapsed
                  ? pick("فتح القائمة", "Expand")
                  : pick("طي القائمة", "Collapse")
              }
            >
              <ChevronLeft
        ...
        {!collapsed && (
          <div className="mt-2 flex items-center justify-between gap-2">
            ...
            <LanguageToggle className="bg-white/10 text-sidebar-foreground hover:bg-white/20" />
          </div>
        )}
      </div>

      <div
        className={`border-b border-sidebar-border transition-all duration-300 ${collapsed ? "h-0 overflow-hidden border-b-0 p-0 opacity-0" : "p-4 opacity-100"}`}
      >
        {switchableAccounts.length > 0 ? (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button
                type="button"
                className="flex w-full cursor-pointer items-center justify-between gap-2 rounded-lg p-2 text-start transition-colors hover:bg-sidebar-accent/50"
                aria-label={pick("تبديل الحساب", "Switch account")}
                title={pick("تبديل الحساب", "Switch account")}
              >
                <span className="min-w-0 flex-1 truncate font-cairo text-sm font-semibold">
                  {user?.fullName}
                </span>
            ...
            <DropdownMenuContent side="bottom" align="start" className="w-64">
              <DropdownMenuLabel>
                {pick("الحسابات المحفوظة", "Saved accounts")}
              </DropdownMenuLabel>
                ...
                >
                  ...
                  <button
                    ...
                    onClick={(event) => {
                      ...
                      forgetRememberedAccount(account.user.id);
                    }}
                    className="grid h-8 w-8 shrink-0 cursor-pointer place-items-center rounded-md text-red-700 hover:bg-red-50 hover:text-red-800"
                    aria-label={pick(
              ...
              <DropdownMenuItem
                className="cursor-pointer gap-3 border-t py-2"
                onClick={() => {
                  prepareAddAccount();
                  navigate("/portal/login?addAccount=1");
                  onNavigate?.();
                }}
              >
                <UserPlus className="h-4 w-4" />
                {pick("إضافة حساب", "Add account")}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ) : (
          <button
            ...
            aria-label={pick("إضافة حساب", "Add account")}
            title={pick("إضافة حساب", "Add account")}
          >
            <span className="min-w-0 flex-1 truncate font-cairo text-sm font-semibold">
              {user?.fullName}
            </span>
            <UserPlus className="h-4 w-4 shrink-0 text-sidebar-foreground/70" />
          </button>
        )}
      </div>
      ...
      <div
        className={`border-t border-sidebar-border transition-all duration-300 ${collapsed ? "p-2" : "p-3"}`}
      >
        <Button
          variant="ghost"
          onClick={logout}
      ...
      {switchingName && (
        <div
          ...
          aria-live="polite"
        >
          <div className="animate-in fade-in zoom-in-95 rounded-2xl border border-[hsl(221_63%_15%/0.12)] bg-white px-10 py-9 text-center shadow-2xl duration-300">
            <span className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-[hsl(221_63%_15%/0.08)] ring-1 ring-[hsl(221_63%_15%/0.12)]">
              <Loader2 className="h-10 w-10 animate-spin text-[hsl(221_63%_15%)]" />
            </span>
            <h2 className="mt-5 text-xl font-bold text-[hsl(221_63%_15%)]">
              {pick("جاري تبديل الحساب", "Switching account")}
            </h2>
            <p className="mt-2 text-sm font-medium text-[hsl(221_63%_15%/0.7)]">
              {switchingName}
            </p>
          </div>
        </div>
...
const DashboardSidebar = () => {
  ...
  const [collapsed, setCollapsed] = useState(
    () => localStorage.getItem("bnan_sidebar_collapsed") === "1",
  );
  ...
  if (!isMobile) {
    return (
      <aside
        className={`relative flex h-full min-h-0 shrink-0 flex-col overflow-hidden border-l border-sidebar-border bg-sidebar text-sidebar-foreground transition-[width] duration-300 ease-in-out ${collapsed ? "w-20" : "w-64"}`}
      >
        <SidebarContent
          collapsed={collapsed}
          onToggle={() =>
    ...
    <>
      <div className="fixed top-0 right-0 left-0 z-40 h-14 bg-sidebar flex items-center justify-between px-3 border-b border-sidebar-border">
        <button
          ...
          className="grid h-10 w-10 place-items-center rounded-lg text-sidebar-foreground hover:bg-sidebar-accent"
          aria-label={pick("فتح القائمة", "Open menu")}
        >
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            size="sm"
            onClick={logout}
            className="h-10 gap-1.5 px-2 text-sidebar-foreground hover:bg-sidebar-accent hover:text-destructive"
            aria-label={pick("تسجيل الخروج", "Log out")}
          >
            <LogOut className="h-4 w-4" />
            <span className="hidden min-[390px]:inline text-xs">
              {pick("خروج", "Log out")}
            </span>
          </Button>
        </div>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent
          side="right"
          className="h-dvh w-72 p-0 bg-sidebar border-sidebar-border"
        >
