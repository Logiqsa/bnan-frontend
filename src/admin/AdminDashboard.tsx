...
function AdminOverview() {
  ...
  return <div className="space-y-6">
    ...
    {stats && <section><div className="mb-3 flex items-center gap-2"><ReceiptText className="h-5 w-5 text-primary" /><h2 className="text-xl font-bold">الوضع المالي</h2></div><div className="grid gap-4 md:grid-cols-3"><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">إيصالات مصرية معلقة</p><p className="mt-2 text-2xl font-bold">{stats.pendingReceipts?.toLocaleString("ar-EG") ?? "—"}</p><Link className="mt-3 inline-block text-sm text-primary hover:underline" to="/admin/payments">فتح المدفوعات</Link></CardContent></Card><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">الاشتراكات المنتهية</p><p className="mt-2 text-2xl font-bold">{stats.expiredSubscriptions?.toLocaleString("ar-EG") ?? "—"}</p><Link className="mt-3 inline-block text-sm text-primary hover:underline" to="/admin/subscriptions">فتح الاشتراكات</Link></CardContent></Card><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">حصص اليوم</p><p className="mt-2 text-2xl font-bold">{stats.todaySessions?.toLocaleString("ar-EG") ?? "—"}</p><Link className="mt-3 inline-block text-sm text-primary hover:underline" to="/admin/classrooms">إدارة الفصول</Link></CardContent></Card></div></section>}
  </div>;
}
