    ...
    <section className="py-2">
      ...
      <Card className="shadow-elegant overflow-hidden"><CardContent className="p-0"><div dir="ltr" className="portal-schedule-layout min-h-[590px]">
        <div dir="rtl" className="portal-calendar-panel p-5 md:p-8">
          <div className="flex items-center justify-between mb-7"><Button size="icon" variant="outline" onClick={()=>changeMonth(1)} aria-label="الشهر التالي"><ChevronRight className="h-4 w-4"/></Button><div className="text-center"><h2 className="font-bold text-lg">{month.toLocaleDateString("ar-EG-u-ca-gregory",{month:"long",year:"numeric"})}</h2><p className="text-xs text-muted-foreground mt-1">{visible.length} حصة</p></div><Button size="icon" variant="outline" onClick={()=>changeMonth(-1)} aria-label="الشهر السابق"><ChevronLeft className="h-4 w-4"/></Button></div>
        ...
        <aside dir="rtl" className="portal-lessons-panel p-5 md:p-8 bg-card"><div className="flex items-center justify-between border-b pb-4 mb-5"><div><h2 className="font-bold">حصص {selectedObject.toLocaleDateString("ar-EG-u-ca-gregory",{weekday:"long"})}</h2><p className="text-sm text-muted-foreground mt-1">{selectedObject.toLocaleDateString("ar-EG-u-ca-gregory",{day:"numeric",month:"long",year:"numeric"})}</p></div><span className="text-xs bg-muted rounded-full px-3 py-1">{selectedLessons.length} حصة</span></div>
