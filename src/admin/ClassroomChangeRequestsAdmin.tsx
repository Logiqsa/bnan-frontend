...
export default function ClassroomChangeRequestsAdmin() {
  ...
  const teachers = useQuery({
    ...
    queryFn: () => adminUsersApi.listAllWithTeacherProfiles(),
    staleTime: 5 * 60_000,
    retry: 1,
  });
  const request = detail.data?.data;
  const approvedTeachers = useMemo(() => (teachers.data || []).filter((teacher) => teacher.teacherId && (!teacher.teacherStatus || teacher.teacherStatus === "approved")), [teachers.data]);
