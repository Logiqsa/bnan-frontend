...
export default function CourseClassroomScheduleAdmin() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([schedule, classroomResponse]) => {
        ...
        setMeetingLink(normalizeZoomState(classroomResponse.data).meetingLink);
      })
  ...
  useEffect(() => {
    if (!loading && !error && rows.length) void loadZoomAvailability();
  }, [error, loadZoomAvailability, loading, rows.length]);

  const add = () => setRows((current) => [...current, { key: crypto.randomUUID(), day: "saturday", startTime: "09:00", endTime: "10:00" }]);
  const update = (key: string, patch: Partial<CourseScheduleSlot>) => setRows((current) => current.map((row) => row.key === key ? { ...row, ...patch } : row));
  const save = async () => {
  ...
  return <DashboardLayout><div dir="rtl" className="mx-auto max-w-5xl space-y-5">
    ...
    {loading ? <div className="grid min-h-64 place-items-center"><Loader2 className="h-7 w-7 animate-spin" /></div> : error ? <Card><CardContent className="py-16 text-center text-destructive">{error}</CardContent></Card> : <Card><CardHeader className="flex-row items-center justify-between"><CardTitle className="flex items-center gap-2"><CalendarDays className="h-5 w-5" />المواعيد الأسبوعية</CardTitle><Button variant="outline" onClick={add}><Plus className="me-2 h-4 w-4" />إضافة موعد</Button></CardHeader><CardContent className="space-y-3">{rows.length === 0 ? <button className="w-full rounded-xl border border-dashed py-14 text-muted-foreground" onClick={add}>لا توجد مواعيد — أضف أول موعد</button> : rows.map((row) => <div key={row.key} className="grid items-end gap-3 rounded-xl border p-4 sm:grid-cols-[1fr_1fr_1fr_auto]"><label className="space-y-2"><Label>اليوم</Label><Select value={row.day} onValueChange={(day) => update(row.key, { day: day as CourseScheduleSlot["day"] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{days.map((day) => <SelectItem key={day} value={day}>{dayNames[day]}</SelectItem>)}</SelectContent></Select></label><label className="space-y-2"><Label>من</Label><Time12Input value={row.startTime} onChange={(startTime) => update(row.key, { startTime })} /></label><label className="space-y-2"><Label>إلى</Label><Time12Input value={row.endTime || ""} allowEmpty onChange={(endTime) => update(row.key, { endTime })} /></label><Button size="icon" variant="ghost" className="text-destructive" onClick={() => setRows((current) => current.filter((item) => item.key !== row.key))}><Trash2 className="h-4 w-4" /></Button></div>)}</CardContent></Card>}
    {!loading && !error && rows.length > 0 && <Card><CardHeader className="flex-row items-center justify-between gap-3"><div><CardTitle className="flex items-center gap-2"><Video className="h-5 w-5" />ربط الدورة بحساب Zoom</CardTitle><p className="mt-2 text-sm text-muted-foreground">{meetingLink ? "هذه المجموعة مرتبطة بحساب Zoom والرابط جاهز للاستخدام." : "تُحسب الإتاحة من جميع مواعيد الجدول لمنع تعارض الحساب مع أي فصل آخر."}</p></div>{!meetingLink && <Button size="sm" variant="outline" onClick={() => void loadZoomAvailability()} disabled={zoomLoading}><RefreshCw className={`me-2 h-4 w-4 ${zoomLoading ? "animate-spin" : ""}`} />تحديث التوفر</Button>}</CardHeader><CardContent className="space-y-3">{meetingLink ? <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-800"><span className="flex items-center gap-2 font-semibold"><CheckCircle2 className="h-5 w-5" />تم ربط المجموعة وإنشاء رابط Zoom</span><Button asChild size="sm"><a href={meetingLink} target="_blank" rel="noreferrer"><ExternalLink className="me-2 h-4 w-4" />فتح الرابط</a></Button></div> : <>{zoomError && <p className="rounded-xl bg-destructive/10 p-3 text-sm text-destructive">{zoomError}</p>}{zoomLoading ? <div className="grid min-h-28 place-items-center"><Loader2 className="h-6 w-6 animate-spin" /></div> : zoomAvailability?.accounts.length ? <div className="grid gap-3 md:grid-cols-2">{zoomAvailability.accounts.map((item) => <div key={item.id} className={`rounded-xl border p-4 ${item.available ? "border-emerald-200 bg-emerald-50/40" : "bg-muted/40"}`}><div className="flex items-center justify-between gap-3"><div><p className="font-bold">{item.name}</p><Badge variant={item.available ? "default" : "secondary"} className={item.available ? "mt-2 bg-emerald-600" : "mt-2"}>{item.available ? "متاح لكل المواعيد" : "غير متاح"}</Badge></div><Button size="sm" disabled={!item.available || Boolean(generatingAccountId)} onClick={() => void createMeeting(item.id)}>{generatingAccountId === item.id && <Loader2 className="me-2 h-4 w-4 animate-spin" />}ربط وإنشاء الرابط</Button></div>{!item.available && item.conflictsWithCurrentClassroom.length > 0 && <div className="mt-3 border-t pt-3 text-xs text-muted-foreground">{item.conflictsWithCurrentClassroom.slice(0, 3).map((conflict, index) => <p key={`${conflict.day}-${conflict.startTime}-${index}`}>{dayNames[conflict.day as CourseScheduleSlot["day"]] || conflict.day}: <span dir="ltr">{conflict.startTime} - {conflict.endTime}</span>{conflict.classroomName ? ` · ${conflict.classroomName}` : ""}</p>)}</div>}</div>)}</div> : zoomAvailability ? <p className="rounded-xl border border-dashed p-8 text-center text-muted-foreground">لا توجد حسابات Zoom مجهزة ومتاحة.</p> : null}</>}</CardContent></Card>}
  </div></DashboardLayout>;
}
