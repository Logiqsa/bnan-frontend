...
export default function StudentSessions() {
  ...
  const classrooms = useMemo(
    () =>
      Array.from(
        new Map(
          lessons.map((lesson) => [lesson.classroom.id, lesson.classroom]),
        ).values(),
      ),
    [lessons],
  );
      ...
      .catch((error) =>
        toast.error(
          (error as Error).message ||
            pick(
              "تعذر تحميل فصول الطالب",
              "Unable to load the student's classes",
            ),
        ),
      )
    ...
    async (id: string) => {
      ...
      try {
        const recordingResponse =
          await classroomRecordingsApi.listRecordings(id);
  ...
  return (
    <DashboardLayout>
      <div className="mx-auto max-w-5xl" dir={isArabic ? "rtl" : "ltr"}>
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-3xl font-bold">
              {pick("تسجيلات الحصص", "Lesson recordings")}
            </h1>
            <p className="mt-1 text-muted-foreground">
              {pick(
                "تسجيلات حصص الفصول المسجل بها.",
                "Recordings from the classes you are enrolled in.",
              )}
            </p>
          </div>
          <Button
                  ...
                  <SelectValue
                    placeholder={
                      loadingClassrooms
                        ...
                        : pick("اختر الفصل", "Select a class")
                    }
                  />
                </SelectTrigger>
                <SelectContent>
        ...
        {loadingClassrooms || loadingSessions ? (
          ...
          <Card>
            <CardContent className="grid min-h-56 place-items-center text-center text-muted-foreground">
              <div>
                ...
                <p>
                  {pick(
                    "لا توجد فصول مسجلة لهذا الطالب.",
                    "No classes are registered for this student.",
                  )}
                </p>
              </div>
            </CardContent>
          ...
          <Card>
            <CardContent className="grid min-h-56 place-items-center text-muted-foreground">
              {pick("لا توجد تسجيلات حتى الآن.", "No recordings yet.")}
            </CardContent>
          </Card>
        ) : (
          <div className="grid gap-4 md:grid-cols-2">
            {recordings.map((recording, index) => {
              ...
              const open = () => {
                if (!videoUrl) return;
                if (recording.shareUrl && !recording.localUrl)
                ...
                setSelectedRecording({
                  sessionName: name,
                  recordingLink: recording.localUrl || videoUrl,
                });
              };
              return (
                <Card
                  ...
                  className={cn(
                    videoUrl &&
                      "cursor-pointer transition-all hover:-translate-y-0.5 hover:border-primary/50 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                  )}
                >
                  <CardContent className="p-5">
                    ...
                    {videoUrl && (
                      <div className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-primary">
                        ...
                        {pick("مشاهدة التسجيل", "Watch recording")}
                      </div>
                    )}
                  </CardContent>
                </Card>
