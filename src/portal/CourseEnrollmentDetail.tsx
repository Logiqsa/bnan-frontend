...
const dayNames: Record<string, string> = { saturday: "السبت", sunday: "الأحد", monday: "الاثنين", tuesday: "الثلاثاء", wednesday: "الأربعاء", thursday: "الخميس", friday: "الجمعة" };
...
export default function CourseEnrollmentDetail() {
  ...
  const scheduleQuery = useQuery({ queryKey: ["student-course-schedule", classroomId], queryFn: () => coursesApi.getSchedule(classroomId), enabled: enrollment?.status === "active" && Boolean(classroomId) });
  ...
  const needsTeacherFallback = typeof course?.teacher === "string" && /^[a-f\d]{24}$/i.test(course.teacher);
  ...
  const teacherName = activeSessionQuery.data?.teacher?.fullName || (needsTeacherFallback
  ...
  return <DashboardLayout><div className="mx-auto max-w-5xl space-y-5">
    ...
    <div className="grid gap-5 md:grid-cols-2"><Card><CardHeader><CardTitle>تفاصيل التسجيل</CardTitle></CardHeader><CardContent className="space-y-3">{course?.description && <p>{course.description}</p>}<p><b>المعلم:</b> {teacherName}</p><p><b>الفصل:</b> {classroom?.name || group?.name || "—"}</p><p><b>السعر:</b> {enrollment.price} {enrollment.currency}</p></CardContent></Card>
    ...
    {enrollment.status === "active" && classroomId && <Card id="course-schedule" className="scroll-mt-6"><CardHeader><CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5" />جدول الدورة</CardTitle></CardHeader><CardContent>{scheduleQuery.isLoading ? <Loader2 className="mx-auto h-6 w-6 animate-spin" /> : scheduleQuery.error ? <p className="text-destructive">{courseError(scheduleQuery.error)}</p> : !scheduleQuery.data?.slots.length ? <p className="text-muted-foreground">لم يحدد الأدمن مواعيد الدورة بعد.</p> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{scheduleQuery.data.slots.map((slot, index) => <div key={`${slot.day}-${slot.startTime}-${index}`} className="rounded-xl border bg-muted/20 p-4"><p className="font-bold">{dayNames[slot.day] || slot.day}</p><p className="mt-1" dir="ltr">{slot.startTime}{slot.endTime ? ` - ${slot.endTime}` : ""}</p></div>)}</div>}</CardContent></Card>}
    <Card><CardHeader><CardTitle>التسجيلات</CardTitle></CardHeader><CardContent>{enrollment.status !== "active" ? <p className="text-muted-foreground">تتاح التسجيلات بعد تفعيل التسجيل.</p> : !classroomId ? <p className="text-muted-foreground">لم يتم تعيين فصل بعد.</p> : recordingsQuery.isLoading ? <p>جاري تحميل التسجيلات...</p> : recordingsQuery.error ? <p className="text-destructive">{courseError(recordingsQuery.error)}</p> : !recordings.length ? <p className="text-muted-foreground">لا توجد تسجيلات حتى الآن.</p> : <div className="grid gap-3 md:grid-cols-2">{recordings.map((recording, index) => { const url = recording.localUrl || recording.recordingLink || recording.shareUrl; return <Button key={recording.sessionId || index} variant="outline" className="h-auto justify-start p-4" disabled={!url} onClick={() => url && (recording.shareUrl && !recording.localUrl ? window.open(recording.shareUrl, "_blank", "noopener,noreferrer") : setSelectedRecording({ sessionName: recording.sessionName, recordingLink: url }))}><Play className="me-2 h-4 w-4"/>{recording.sessionName || "تسجيل حصة"}</Button>; })}</div>}</CardContent></Card>
      ...
      <TabsContent value="chat" className="mt-0">
        {enrollment.status === "active" && classroomId && <CourseClassroomChat classroomId={classroomId} />}
      </TabsContent>
    </Tabs>
