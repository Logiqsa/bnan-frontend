...
export default function ClassroomChangeRequestsAdmin() {
  ...
  const requestClassroom = request?.classroom as Record<string, unknown> | string | undefined;
  const requestCurriculumId = requestClassroom && typeof requestClassroom === "object" ? idOf(requestClassroom.curriculum) : "";
  const approvedTeachers = useMemo(() => {
    if (!requestCurriculumId) return [];
    return (teachers.data || []).filter((teacher) => teacher.teacherId
      && teacher.teacherStatus === "approved"
      && teacher.curriculums?.some((curriculum) => idOf(curriculum) === requestCurriculumId));
  }, [requestCurriculumId, teachers.data]);
